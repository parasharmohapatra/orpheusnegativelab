...
class ToneCurveEditor(QMainWindow):
    ...
    def update_image_after_slider(self):
        ...
        if self.pending_slider_values and self.processor.inverted_image is not None:
            ...
            # Update the image cache with the new processed image
            if self.current_index >= 0 and self.current_index < len(self.image_files):
                current_key = str(self.image_files[self.current_index])
                if current_key in self.image_cache:
                    self.image_cache[current_key]['processed_image'] = processed_image
    ...
    def load_current_image(self):
        ...
        try:
            ...
            if image_key in self.image_cache:
                cached_data = self.image_cache[image_key]
                
                # Use the cached processor
                self.processor = cached_data['processor']
                
                # Load image settings to update UI sliders
                ...
                if cached_data['processed_image'] is not None and not self.settings_changed(image_key):
                    # Use cached processed image directly - this is the fast path
                    processed_image = cached_data['processed_image']
                    
                    # Get tone curves for histogram (either cached or calculate)
                    if cached_data.get('tone_curves') is not None:
                        tone_curves = cached_data['tone_curves']
                    else:
                        tone_curves = self.calculate_tone_curves_only(cached_data['inverted_image'])
                        cached_data['tone_curves'] = tone_curves
                else:
                    # Need to generate or regenerate the processed image
                    ...
                    # Update the cache with the new processed image and tone curves
                    cached_data['processed_image'] = processed_image
                    cached_data['tone_curves'] = tone_curves
                    self.update_cache_settings_hash(image_key)
            else:
                # Fallback to original method if image not in cache yet
                ...
                if inverted_image is not None:
                    ...
                    self.image_cache[image_key] = {
                        ...
                        'settings_hash': self.get_settings_hash(image_key)
                    }
    ...
    def load_next_image(self):
        ...
        if self.current_index < len(self.image_files) - 1:
            ...
            # Update the cache with current processed image
            current_key = str(self.image_files[self.current_index])
            if current_key in self.image_cache:
                self.image_cache[current_key]['processed_image'] = self.processor.processed_image
                # Also update the tone curves
                tone_curves = self.calculate_tone_curves_only(self.processor.inverted_image)
                self.image_cache[current_key]['tone_curves'] = tone_curves
                # Update settings hash
                self.update_cache_settings_hash(current_key)
    ...
    def rotate_current_image(self):
        ...
        if self.processor.processed_image is not None:
            ...
            # Update the cache with the rotated image
            if self.current_index >= 0 and self.current_index < len(self.image_files):
                current_key = str(self.image_files[self.current_index])
                if current_key in self.image_cache:
                    self.image_cache[current_key]['processed_image'] = rotated_image
                    # Update the settings hash to match new rotation
                    self.update_cache_settings_hash(current_key)
    ...
    def flip_current_image(self):
        ...
        if self.processor.processed_image is not None:
            ...
            # Update the cache with the flipped image
            if self.current_index >= 0 and self.current_index < len(self.image_files):
                current_key = str(self.image_files[self.current_index])
                if current_key in self.image_cache:
                    self.image_cache[current_key]['processed_image'] = flipped_image
                    # Update the settings hash to match new flip state
                    self.update_cache_settings_hash(current_key)
    ...
    def reset_current_image(self):
        ...
        if self.processor.inverted_image is not None:
            ...
            # Update the image cache with the reset image
            if self.current_index >= 0 and self.current_index < len(self.image_files):
                current_key = str(self.image_files[self.current_index])
                if current_key in self.image_cache:
                    self.image_cache[current_key]['processed_image'] = processed_image
            ...
            if key and key in self.image_settings:
                ...
                # Update the settings hash (will be None since settings were deleted)
                self.update_cache_settings_hash(key)
    ...
    def load_images_from_directory(self):
        ...
        if self.image_files:
            # Clear existing cache before loading new images
            self.image_cache = {}
            
            # Create and show progress dialog
            self.progress_dialog = QMessageBox()
            self.progress_dialog.setWindowTitle("Loading Images")
            self.progress_dialog.setText("Preloading images...")
    ...
    def preload_images(self):
        """Preload all images from the directory into memory"""
        total_images = len(self.image_files)
        
        # Update progress dialog
        self.statusBar.showMessage(f"Preloading {total_images} images...")
        
        # Preload first image immediately so user can start working
        if total_images > 0:
            raw_file = self.image_files[0]
            key = str(raw_file)
            self.preload_single_image(raw_file, key)
            
        # Start a background timer to load the rest of the images
        # This allows the UI to remain responsive
        self.preload_next_index = 1
        ...
        if total_images <= 1:
            if self.progress_dialog:
                ...
                self.progress_dialog = None
            self.statusBar.showMessage(f"Loaded {total_images} image", 3000)
            return
            
        # Setup timer for preloading remaining images
        self.preload_timer = QTimer()
        self.preload_timer.timeout.connect(self.preload_next_image)
        self.preload_timer.start(10)  # Short interval to load quickly
    
    def preload_next_image(self):
        """Load the next image in the preloading queue"""
        if self.preload_next_index >= len(self.image_files):
            # Done loading all images
            self.preload_timer.stop()
            if self.progress_dialog:
                self.progress_dialog.close()
                self.progress_dialog = None
            self.statusBar.showMessage(f"All {len(self.image_files)} images loaded", 3000)
            return
            
        # Update progress
        progress_pct = int((self.preload_next_index / len(self.image_files)) * 100)
        self.statusBar.showMessage(f"Preloading images: {self.preload_next_index}/{len(self.image_files)} ({progress_pct}%)")
        
        if self.progress_dialog:
            self.progress_dialog.setText(f"Preloading images: {self.preload_next_index}/{len(self.image_files)} ({progress_pct}%)")
        
        # Load the next image
        raw_file = self.image_files[self.preload_next_index]
        key = str(raw_file)
        self.preload_single_image(raw_file, key)
    ...
    def preload_single_image(self, raw_file, key):
        """Load a single image into the cache"""
        try:
            # Create a dedicated processor for this image
            processor = ToneCurveProcessor(str(raw_file))
            ...
            if inverted_image is not None:
                ...
                if key in self.image_settings:
                    # Apply stored settings to the processor
                    settings = self.image_settings[key]
                    if 'r_left_slope' in settings:
                        processor.r_left_slope = settings['r_left_slope']
                    if 'r_right_slope' in settings:
                        processor.r_right_slope = settings['r_right_slope']
                    if 'g_left_slope' in settings:
                        processor.g_left_slope = settings['g_left_slope']
                    if 'g_right_slope' in settings:
                        processor.g_right_slope = settings['g_right_slope']
                    if 'b_left_slope' in settings:
                        processor.b_left_slope = settings['b_left_slope']
                    if 'b_right_slope' in settings:
                        processor.b_right_slope = settings['b_right_slope']
                    if 'exposure' in settings:
                        processor.exposure = settings['exposure']
                    if 'contrast' in settings:
                        processor.contrast = settings['contrast']
                    if 'rotation' in settings:
                        processor.rotation_angle = settings['rotation']
                    if 'flipped' in settings:
                        processor.is_flipped = settings['flipped']
                    if 'zoom_factor' in settings:
                        processor.zoom_factor = settings['zoom_factor']
                    
                    # Pre-generate the processed image with these settings
                ...
                self.image_cache[key] = {
                    ...
                    'inverted_image': inverted_image,
                    'processed_image': processed_image,
                    'tone_curves': None,  # Will store tone curves for histogram
                    'settings_hash': self.get_settings_hash(key)  # Track settings hash to detect changes
                }
        except Exception as e:
            print(f"Error preloading image {raw_file}: {e}")
    
    def get_settings_hash(self, key):
        """Create a hash of the current settings to detect changes"""
        if key in self.image_settings:
            settings = self.image_settings[key]
            # Create a simple string representation to use as hash
            hash_str = str(settings)
            return hash(hash_str)
        return None
    
    def settings_changed(self, key):
        """Check if settings have changed since image was cached"""
        if key in self.image_cache:
            current_hash = self.get_settings_hash(key)
            cached_hash = self.image_cache[key].get('settings_hash')
            return current_hash != cached_hash
        return True  # If not in cache, consider settings changed
    
    def update_cache_settings_hash(self, key):
        """Update the settings hash in the cache"""
        if key in self.image_cache:
            self.image_cache[key]['settings_hash'] = self.get_settings_hash(key)
